...
library(caTools)
set.seed(3000)
spl = sample.split(stevens$Reverse, SplitRatio = 0.7) #데이터 스플릿
Train = subset(stevens, spl = TRUE) # 트레이닝 셋
Test = subset(stevens, spl = FALSE) # 테스트 셋

install.packages("rpart")
library(rpart)
install.packages("rpart.plot") # rpart package 다운
library(rpart.plot)
